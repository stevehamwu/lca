...
def test(topK=1, matchmode='precise'):
	qalist = read()
	qa = qalist[0]
	print(qa.query)
	print(qa.candidates)
	print(qa.answer)
	for cword in qa.cwords:
		print(cword)
	print(qa.nw)
	print(qa.nc)
	fcq = qa.getFCQ()
	...
	for mf in max_fcq:
		if matchmode == 'precise':
			...
			if qa.answer.strip().__contains__(mf[0].strip()) or mf[0].strip().__contains__(qa.answer.strip()):
				...
				break
	print('answer:', qa.answer,'top ', topK, 'max_lca: ', max_fcq, correct)
	print(qa.idf('窃读'), qa.idf('作者'), qa.idf('作品'))
	print(qa.En('林海音', '窃读'), qa.En('林海音', '作者'), qa.En('林海音', '作品'))
	print(qa.co_degree('林海音', '窃读'), qa.co_degree('林海音', '作者'), qa.co_degree('林海音', '作品'))
	print(qa.getF('林海音'))
